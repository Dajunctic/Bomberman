...
public class Mobile extends Entity{
    ...
    //extension
    public boolean checkCollision(double ref_x, double ref_y, int losen) {
        // nới lỏng checker
        double margin = losen;
        // có đấy bạn ạ
        if(ref_x < 0 || ref_y < 0
        ...
        for (int i = tileStartX; i <= tileEndX; i++) {
            for (int j = tileStartY; j <= tileEndY; j++) {
                ...
                Rectangle tileRect = new Rectangle(tileX + margin, tileY + margin, Sprite.SCALED_SIZE - margin, Sprite.SCALED_SIZE - margin);
